...
public class A_LancerRetrat : i_EnemyActions
{
    ModelE_Lancer _e;
    ...
    public void Actions()
    {
        ...
        if (_e.onRetreat && _e.timeToRetreat > 0 && d < maxD && _e.animClipName == _e.view.animDictionary[ViewerE_Lancer.EnemyMeleeAnim.Retreat])
        {
        ...
        if (_e.onRetreat && _e.animClipName != _e.view.animDictionary[ViewerE_Lancer.EnemyMeleeAnim.Attack1] && _e.animClipName != _e.view.animDictionary[ViewerE_Lancer.EnemyMeleeAnim.WalkForward] && _e.animClipName != _e.view.animDictionary[ViewerE_Lancer.EnemyMeleeAnim.Blocked] && _e.timeToRetreat > 0 && d > maxD)
        {


            var obs = Physics.OverlapSphere(_e.transform.position, 0.5f, _e.layerEntites).Where(x => x.GetComponent<ModelE_Lancer>()).Select(x => x.GetComponent<ModelE_Lancer>()).Where(x => x != _e).ToList();
            obs.Remove(_e);
    ...
    public A_LancerRetrat(ModelE_Lancer e)
    {
        _e = e;
    }
}
