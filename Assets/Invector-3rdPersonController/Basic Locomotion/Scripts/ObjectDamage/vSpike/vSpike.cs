        ...
        void OnCollisionEnter(Collision collision)
        {
            if (collision.rigidbody != null && collision.collider.GetComponent<vCharacterController.vDamageReceiver>() != null && !inConect)
            {
                ...
                if (condition)
                {
                    ...
                    var ichar = collision.collider.GetComponent<vCharacterController.vDamageReceiver>();
                    if (ichar) ichar.ragdoll.iChar.ChangeHealth((int)-ichar.ragdoll.iChar.currentHealth);
                }
            }
        }
