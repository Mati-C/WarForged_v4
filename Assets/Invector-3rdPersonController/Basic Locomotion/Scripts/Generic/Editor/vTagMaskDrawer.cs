...
public class vTagMaskDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        var fieldWidth = EditorGUIUtility.labelWidth;
        var rect = position;
        rect.height = EditorGUIUtility.singleLineHeight;
        rect.width = fieldWidth;
        GUI.Label(rect, label);
        rect.x = (position.x + fieldWidth);
        rect.width = (position.width - (fieldWidth));
        var tags = GetTagList(prop);
        if (EditorGUI.DropdownButton(rect, new GUIContent(prop.arraySize == 0 ? "Nothing" : prop.arraySize <= 4 ? Get4FirstNames(prop) : "Mixed ..."), FocusType.Passive, EditorStyles.popup))
        {
